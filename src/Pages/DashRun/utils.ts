...
export const KMeansCentroidsSearch = (distances: number[]) => {
  const centroids = [
    [
      ...
      39.69141225804538, 38.27270597176409,
    ],
    [
      71.86672601184169, 88.76003536712001, 107.09756812003087,
      103.72602043189468, 88.34780049860731,
    ],
  ];
...
export const createKeyMap = (
  handPoseEstimations: handPoseDetection.Keypoint[],
) =>
  handPoseEstimations.reduce<Record<string, handPoseDetection.Keypoint>>(
    (acc, item) => {
      acc[item.name!] = item;
      return acc;
    },
...
export const getHandPoseEstimationsDistances = (
  data: Record<string, handPoseDetection.Keypoint>,
) => {
