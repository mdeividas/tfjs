...
export default class Circle {
    ...
    isCursorOverCircle(x: number, y: number) {
        const distance = Math.sqrt(Math.pow(this.#params.x - x, 2) + Math.pow(this.#params.y - y, 2));
    ...
    draw() {
        ...
        this.#context.fillStyle = this.#active ? this.#params.color[1] : this.#params.color[0];
        this.#context.fill();
        this.#context.closePath();

        if (this.#activating) {
            this.#activatingProgress += 1000 / 35;
            const progress = Math.min(interpolate(this.#activatingProgress, 0, Circle.ACTIVATION_TIME, 0, 1), 1);

            this.#context.beginPath();
            this.#context.arc(this.#params.x, this.#params.y, this.#params.radius, 0, 2 * Math.PI * progress);
