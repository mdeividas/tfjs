...
export const IntroView: React.FC<IProps> = (props) => {
    ...
    return (
        <div className="flex flex-col gap-8 justify-center items-center mt-2 mb-8 ">
            <p className="text-slate-700 text-justify w-full md:w-2/3">{props.text}</p>

            <button onClick={props.onClick} className="text-3xl mt-8 mb-3 font-bold text-center uppercase">
                Try now! ⬇️
            </button>
            ...
            {showModal && <HowToPlay url={props.htpLink} onClose={() => setShowModal(false)} />}
        </div>
    );
};
